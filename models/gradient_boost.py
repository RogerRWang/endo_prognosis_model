...
class GradientBoostModel:
    ...
    def visualize(self, n: int):
        ...
        for i in range(n):
            ...
            plot_tree(tree_model, feature_names=self._X_train_cleaned.columns, class_names=['Success', 'Failure'], filled=True, ax=axes[i])
            axes[i].set_title(f'Tree {i + 1}')

        plt.tight_layout()
        plt.savefig('./output/visualizations/gradient_boost_trees.png')
